...
export class NfcSoundLibraryComponent implements OnDestroy {
  ...
  protected readonly activeTab = signal<SoundTab>('mine');
  ...
  protected async uploadRecording() {
    ...
    try {
      const sound = await firstValueFrom(this.api.uploadSound(blob, `sound.${this.extensionFor(blob.type)}`, this.recordingName()));
