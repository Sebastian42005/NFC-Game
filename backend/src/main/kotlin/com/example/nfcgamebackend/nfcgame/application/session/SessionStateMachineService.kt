...
import com.example.nfcgamebackend.nfcgame.persistence.entity.NfcFlowNode
...
import com.example.nfcgamebackend.nfcgame.persistence.entity.NfcSessionAccount
import com.example.nfcgamebackend.nfcgame.persistence.entity.NfcSessionRound
import com.example.nfcgamebackend.nfcgame.persistence.entity.NfcSessionTeam
import com.example.nfcgamebackend.nfcgame.persistence.entity.NfcSessionTeamMember
...
import com.example.nfcgamebackend.nfcgame.persistence.repository.NfcFlowEdgeRepository
...
import com.example.nfcgamebackend.nfcgame.persistence.repository.NfcSessionRoundRepository
...
import com.example.nfcgamebackend.nfcgame.persistence.repository.NfcSessionValueRepository
...
data class StateMachineResult(
    ...
    val sound: FlowSound? = null,
)

data class FlowSound(
    val soundId: UUID,
    val name: String?,
    val target: String,
)
...
class SessionStateMachineService(
    ...
    private fun handleGameCard(device: NfcDevice, card: NfcCard): StateMachineResult {
        ...
        if (activeSession.status in setOf(SessionStatus.BUILDING_TEAMS, SessionStatus.LOBBY, SessionStatus.READY)) {
            if (activeSession.currentStateKey == TEAM_SIZE_STATE) {
                ...
                return startSession(activeSession) ?: StateMachineResult(
                    session = activeSession,
                    screen = buildScreen(activeSession),
    ...
    private fun continueRunningFlowAfterPlayerScan(
        ...
        repeat(MAX_RUNTIME_STEPS) {
            val node = nodeId?.let { flowNodeRepository.findById(it).orElse(null) }
                ?: return finishFlowFallback(session, teams, winningTeamId, timelineMessage, popup, sound)
            when (node.type) {
    ...
    private fun startSession(session: NfcGameSession): StateMachineResult? {
        session.status = SessionStatus.RUNNING
        ...
        val firstNodeId = template?.let { firstFlowNodeAfterStart(it) }
            ?: flowNodeRepository.findAllByGameTemplateIdOrderBySortOrderAsc(requireNotNull(session.gameTemplateId))
                ...
                ?.id
        return if (firstNodeId != null) {
            enterRunningFlowNode(session, firstNodeId, emptyMap(), listOf("BEEP_START"))
        } else {
    ...
    private fun soundForNode(node: NfcFlowNode): FlowSound? {
        val config = readMap(node.configJson)
        val soundId = config["soundId"]?.toString()?.let { runCatching { UUID.fromString(it) }.getOrNull() }
            ?: return null
        val target = config["playTarget"]?.toString()?.uppercase()?.takeIf { it in setOf("DEVICE", "WEBSITE", "BOTH") }
            ?: "DEVICE"
        val name = config["soundName"]?.toString()?.takeIf { it.isNotBlank() }
        return FlowSound(soundId = soundId, name = name, target = target)
    }
            ...
            EventType.TOUCH_CONFIRM -> {
                ...
                if (wantsStartGame) {
                    ...
                    return startSession(session)
                        ?: StateMachineResult(session = session, screen = buildScreen(session), effects = listOf("BEEP_START"))
                }
                val rawValue = (payload["value"] as? Number)?.toInt() ?: payload["value"]?.toString()?.toIntOrNull()
                if (rawValue != null) {
                    team.targetSize = rawValue.coerceIn(1, 20)
